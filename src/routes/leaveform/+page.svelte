...
import LeavebalanceComponent from '$lib/component/leaveform/leavebalance.svelte'
  ...
  let employee_id
  onMount(()=>{
    employee_id=$page.url.searchParams.get('employeeid')?$page.url.searchParams.get('employeeid'):-1
  })
...
<div>
  {#if employee_id}
    {#if employee_id != -1}
    <LeavebalanceComponent employee_id={employee_id}/>{/if} 
    ...
    <LeaveformList employee_id={employee_id} bind:reloadList={reloadList} let:currRecord={record}>
      ...
      <Popover label="Remove" position="top"><button on:click={()=>{recordIdToRemove=record.id}} class="btn-danger" style="padding:0.2em;">
